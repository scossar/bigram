...
)  # using 0.1 instead of 0.01 (for demonstration purposes)
...
bnmean_running = torch.zeros(
    (1, n_hidden)
)  # mean will start near 0 (due to initialization method)
bnstd_running = torch.ones(
    (1, n_hidden)
)  # std will start near 1 (due to initialization method)
...
)  # essentially replicating dcounts_sum in a (batch_size, 27) tensor
# the derivative of the natural exponential function is the function itself
dnorm_logits = (
    norm_logits.exp() * dcounts
)  # this could/should be simpflied to `counts * dcounts` (since `counts = norm_logits.exp()`)
...
print(
    "dlogits[0] * n", dlogits[0] * n
)  # scaling by n here to remove averaging and make results easier to visualize
...
db2 = dlogits.sum(
    0
)  # don't use keepdim=True here the result should have torch.size([27]) not torch.size([1, 27])
# g\prime(z) = 1 - a^2
dhpreact = (1 - h**2) * dh  # approximate: True
dbngain = (bnraw * dhpreact).sum(0, keepdim=True)
dbnbias = dhpreact.sum(0, keepdim=True)
dbnraw = bngain * dhpreact
dbnvar_inv = (bndiff * dbnraw).sum(
    0, keepdim=True
)  # sum(0) because it's summing across examples shape ([1, 200])
